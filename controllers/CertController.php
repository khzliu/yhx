...
class CertController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        //实现文章分页
        $query = ArticleCert::find()->select(['id', 'title' ,'type'])->where(['type' => $type[0]])->orderBy('id DESC');
    ...
    public function actionArticle()
    {
        ...
        $previousArticle = ArticleCert::find()->select(['id', 'title', 'type'])->where('id>' . $id)->orderBy('id ASC')->limit(1)->one();
        //下一条:select * from 表 where 数据id>@当前显示数据id order by 数据_id desc) limit 1
        $nextArticle = ArticleCert::find()->select(['id', 'title', 'type'])->where('id<' . $id)->orderBy('id DESC')->limit(1)->one();
